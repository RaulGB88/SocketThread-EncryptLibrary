...
JOKE = 'Cual es el colmo de un programador? No poder programar sus vacaciones.'
BYE = 'bye'
    ...
    with lock:
        with socket_atiende:
            ...
            while not close:
                ...
                message = lib.encryptCesarASCI.encrypt(JOKE, key)
                print("[Server] Mensaje Cifrado:", message)

                # Send a Joke to Client and get message to client
                message = lib.socketLibrary.messagesText(socket_atiende, message)

                # Send a Key to Client and get message to client
                message = lib.socketLibrary.messagesText(socket_atiende, key)

                #if message == finished_message or message == BYE:
                close = True
